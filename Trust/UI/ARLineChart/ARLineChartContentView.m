...
@property (nonatomic,strong)ARColorModel *colorModel;
...
- (id)initWithFrame:(CGRect)frame dataSource:(NSArray*)dataSource colorModel:(ARColorModel *)colorModel
{
...
//        //************ end test *********************
        ...
        self.xyTextFont = [UIFont systemFontOfSize:13];
        self.xyTextColor = colorModel.yColor ?: [UIColor lightGrayColor];
        self.dataLineColor = colorModel.formColor ?: [UIColor lightGrayColor];
//        [UIColor lightGrayColor];
        ...
        //y轴上每一个Step的距离
        self.yPerStepHeight = (self.originPoint.y - self.leftTopPoint.y) / (DEFAULT_Y_COUNT) - 1;
        //y轴上最大Step数
...
- (void)buildXYSetpArray:(NSInteger)xStepCount yStepCount:(NSInteger)yStepCount
{
    ...
    if (self.dataSource.count >= 2) {
        ...
        float xPer = (xMax - xMin + 1 ) / xStepCount ;
        self.xPerValue = xPer;
//        for (int i=0; i<xStepCount; i++) {
        for (int i=0; i<xStepCount; i++) {

            [self.xArray addObject:[NSNumber numberWithFloat: (xMin + i * xPer)]];
//            [self.xArray addObject:[NSNumber numberWithFloat: (xMin + xPer + i * xPer)]];
        }
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (NSInteger index=0; index < self.xArray.count; index++) {
        ...
        if (xPosition >= self.originPoint.x && xPosition < self.rightBottomPoint.x) {
            ...
            //画x轴文字
            [self.colorModel.xColor ?: self.xyTextColor set];
...
//            CGContextSetLineDash(context, 0.0, dashPattern, 2); //虚线效果
            [ARLineChartCommon drawLine:context
                              ...
                              lineColor:self.colorModel.formColor ?: self.dataLineColor];
        }
    }
    ...
    for (NSInteger i = 0; i < self.y1Array.count; i++) {
        ...
        if (y1Position < self.originPoint.y && y1Position > self.leftTopPoint.y) {
            //画左y轴文字
            [self.colorModel.yColor ?: _xyTextColor set];
            CGSize title1Size = [valStr sizeWithFont:self.xyTextFont];
...
//            float xPosition2 = self.originPoint.x + ((self.xPerStepWidth * item2.xValue) / x1) + self.contentScroll.x;
            float xPosition2 = self.originPoint.x + (self.xPerStepWidth * (i + 1)) + self.contentScroll.x;
            ...
            {
                [ARLineChartCommon drawLine:context startPoint:startPoint endPoint:endPoint lineColor:self.colorModel.lineY1Color ?: line1Color];
            }
            //画折线2
