...
protocol MLMyWalletViewControllerDelegate: class {
...
class MLMyWalletViewController: UIViewController {
    ...
    lazy var titleLabel: UILabel = {
        ...
        titleLabel.text = "ML.My.Wallet".localized()
    ...
    weak var delegate: MLMyWalletViewControllerDelegate?
    ...
    func setup() {
        ...
        NSLayoutConstraint.activate([
            ...
            walletsView.topAnchor.constraint(equalTo: titleLabel.bottomAnchor, constant: kAutoLayoutHeigth(48)),
            ...
            walletsView.heightAnchor.constraint(equalToConstant: kAutoLayoutHeigth(124)),
            ...
            settingView.heightAnchor.constraint(equalToConstant: kAutoLayoutHeigth(200)),
    ...
    func delete(wallet: WalletInfo) {
        ...
        keystore.delete(wallet: wallet) { [weak self] result in
            ...
            switch result {
            case .success:
                self.delegate?.didDeleteAccount(account: wallet, in: self.toViewController() as! MLMyWalletViewController)
...
extension MLMyWalletViewController: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if tableView == walletsView {
            ...
            delegate?.didSelect(wallet: viewModel.wallet, account: viewModel.account, in: self.toViewController() as! MLMyWalletViewController)
            return
        }
        delegate?.didPresentVC(pushType: self.mlsettingModel.pushTypes[indexPath.row])
    }
}
extension MLMyWalletViewController: MLWalletViewCellDelegate {
    func didPress(viewModel: WalletAccountViewModel, in cell: MLWalletViewCell) {
        delegate?.didSelectForInfo(wallet: viewModel.wallet, account: viewModel.account, in: self.toViewController() as! MLMyWalletViewController)
    }
...
extension MLMyWalletViewController: WalletsViewModelProtocol {
    func update() {
        viewModel.refresh()
        walletsView.reloadData()
        settingView.reloadData()
